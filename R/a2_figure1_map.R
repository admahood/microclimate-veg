...
pm <- ggplot() +
  geom_raster(data = mef_hill, aes(x=x, y=y, fill = hillshade)) +
  # geom_tile(data = wc_ll, aes(x=x,y=y), color = "red", fill = "transparent") +
  ...
  coord_sf(xlim = c(-105.105, -105.015),
           ylim = c(39.07, 39.11),
           expand = F) +
  ggtitle("Manitou Experimental Forest") +
  theme_void() +
  theme(axis.title = element_blank(),
        legend.position = 'none');pm
#
#
...
pv <- ggplot() +
  geom_raster(data = val_hill, aes(x=x, y=y, fill = hillshade)) +
  ...
  geom_sf_text(data = locations_val |>
                 filter(id %in% c("vg14", 'vg15')), aes(label = id), nudge_x = 500) +
  coord_sf(xlim = c(-106.565, -106.45),
           ylim = c(35.83, 35.89),
           expand = F)+
  ggtitle("Valles Caldera National Preserve") +
  theme_void() +
  theme(axis.title = element_blank(),
        legend.position = 'none');pv
...
inset_val <- read_csv("data/cleaned_bm/vall_dec22.csv") |>
  ...
  ylab("T (deg C)") +
  theme_clean() +
  theme(legend.title = element_blank());inset_val
...
inset_mef <- read_csv("data/cleaned_bm/mef_febmay24.csv") |>
  ...
  ylab("T (deg C)") +
  theme_clean() +
  theme(legend.title = element_blank());inset_mef


pi <- cowplot::ggdraw(ggarrange(pm, pv, ncol=1)) +
  cowplot::draw_plot(inset_mef, .05, .52, .6, .15) +
  cowplot::draw_plot(inset_val, .38, .3, .6, .15)

ggsave(plot = pi, filename = 'out/map_w_inset.png', width = 6.5, height = 8.5, bg="white")
